...
# author: Kennethreith
# v1.1 Leo
#   init the database instance with sqlalchemy instance
#   added conversion when convert the result to dictionary
#   compatiable for process the dictionary and position paramters when do the query
#   automatically convert the underscore names in python to camelcase in front-end
#   delete bulk query from file

# Todo
# return the {} if the query result is none
...
class Record(object):
    ...
    def _named_as_camelcase(self,key):
        words = key.split('_')
...
class RecordCollection(object):
    ...
    def all(self, camelcase = True, to_dict=False, as_ordereddict=False):
        ...
        if to_dict:
            return [r.to_dict(camelcase = camelcase) for r in rows]
        elif as_ordereddict:
            return [r.to_dict(camelcase = camelcase,ordered=True) for r in rows]

        return rows

    def to_dict(self, camelcase = True, ordered=False):
        return self.all(camelcase = camelcase, to_dict=not(ordered), as_ordereddict=ordered)

    def first(self, camelcase = True, default=None, to_dict=False, as_ordereddict=False):
        ...
        # Cast and return.
        if to_dict:
            return record.to_dict(camelcase = camelcase)
        elif as_ordereddict:
            return record.to_dict(camelcase = camelcase, ordered=True)
        else:
            return record
...
class Database(object):
    ...
    def __init__(self, db = None):
        ...
        if not self.db:
            raise ValueError('You must provide a SQLAlchemy instance.')
    ...
    def get_table_names(self, internal=False):
        ...
        return inspect(self.db.engine).get_table_names()

    def query(self, query, *pos_params , fetchall=False, **dic_params):
        ...
        cursor = self.db.engine.execute(text(query), *pos_params ,**dic_params,) # TODO: PARAMS GO HERE
    ...
    def bulk_query(self, query, multiparams):
        ...
        self.db.engine.execute(text(query), multiparams)
    ...
    def paginate(self,query,*pos_params , fetchall=False, **dic_params):

        page = pos_params[0].get('page', 1)
        page_size = pos_params[0].get('page_size', 4)
        ...
        pagination = namedtuple('Pagination',['total_number', 'page', 'page_size', 'data'])
        total_count_sql = 'SELECT count(1) AS counts FROM ({}) t'.format(query)
        paginate_sql = '{} LIMIT :limit OFFSET :offset'.format(query)
        counts = self.db.engine.execute(text(total_count_sql), *pos_params ,**dic_params).first().counts
        pos_params[0].update(limit_offset)

        cursor = self.db.engine.execute(text(paginate_sql), *pos_params, **dic_params)
        row_gen = (Record(cursor.keys(), row) for row in cursor)

        # Convert psycopg2 results to RecordCollection.
        data = RecordCollection(row_gen)

        # Fetch all results if desired.
        if fetchall:
            data.all()

        pagination.total_number = counts
        pagination.page = page
        pagination.page_size = page_size
        pagination.data = data

        return pagination
        # cursor = self.db.engine.execute(text(query), *pos_params ,**dic_params)
